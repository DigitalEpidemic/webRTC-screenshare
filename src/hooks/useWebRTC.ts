...
export function useWebRTC({ roomId }: UseWebRTCProps): UseWebRTCResult {
  ...
  const connectToSignalingServer = useCallback(() => {
    ...
    try {
      ...
      socket.current.on('stream-update', (data: { userId: string; isStreaming: boolean }) => {
        ...
        console.log(
          `Received stream update: ${streamingUserId} ${isStreaming ? 'started' : 'stopped'} streaming`
        );
        ...
        if (isStreaming) {
          ...
          if (attempts < 2) {
            ...
            if (peerConnections.current[streamingUserId]) {
              console.log(
                `Recreating connection with ${streamingUserId} to receive their stream (attempt ${attempts + 1})`
              );
              ...
              setTimeout(() => {
                ...
                pc.oniceconnectionstatechange = () => {
                  console.log(
                    `ICE connection state with ${streamingUserId}: ${pc.iceConnectionState}`
                  );

                  // Add more robust recovery for failed connections
                  if (pc.iceConnectionState === 'failed') {
                    console.log(
                      `ICE connection failed with ${streamingUserId}, attempting recovery`
                    );
                    try {
                      pc.restartIce();
                    } catch (err) {
                ...
                pc.ontrack = event => {
                  ...
                  if (event.streams && event.streams.length > 0) {
                    ...
                    console.log(
                      `Stream has ${tracks.length} tracks:`,
                    ...
                    if (stream.getVideoTracks().length > 0) {
                      ...
                      console.warn(
                        `Stream from ${streamingUserId} has no video tracks, may not be visible`
                      );
                      storeStream(streamingUserId, stream);
                    }
                  }
                ...
                if (localStreamRef.current) {
                  localStreamRef.current.getTracks().forEach(track => {
                    console.log(
                      `Adding our ${track.kind} track to new connection with ${streamingUserId}`
                    );
            ...
            console.log(
              `Already attempted ${attempts} recreations for ${streamingUserId}, waiting for ping mechanism`
            );
          }
        } else {
  ...
  const handleSignalingMessage = useCallback((message: SignalingMessage) => {
    ...
    if (message.type === 'ping' && message.needsStream) {
      ...
      if (pc && localStreamRef.current) {
        ...
        console.log(
          `Cannot respond to stream request from ${peerId} - no local stream or peer connection`
        );
      }
  ...
  const shareScreen = useCallback(async (): Promise<MediaStream | null> => {
    try {
      ...
      setError(
        'Failed to share screen: ' + (error instanceof Error ? error.message : String(error))
      );
      return null;
    }
  }, [roomId]);
        ...
        pc.oniceconnectionstatechange = () => {
          ...
          if (pc.iceConnectionState === 'failed') {
            if (streamMetadata.current[peerId]) {
              ...
              console.log(
                `ICE connection with ${peerId} failed (attempt ${failures}), attempting to restart`
              );
          ...
          if (pc.iceConnectionState === 'connected' || pc.iceConnectionState === 'completed') {
            const peerHasPlaceholderButNoStream =
              peerId in peerStreams && peerStreams[peerId] === null;

            if (peerHasPlaceholderButNoStream && socket.current && socket.current.connected) {
              console.log(
                `Connection with ${peerId} established but no stream received. Sending stream request ping.`
              );
  ...
  const handleOffer = useCallback(
    (message: SignalingMessage) => {
      ...
      if (
        ...
        setTimeout(() => {
          if (streamMetadata.current[peerId]?.pendingOffer && peerConnections.current[peerId]) {
            ...
            if (peerConnections.current[peerId].signalingState === 'stable') {
              ...
              if (queuedOffer) {
                peerConnections.current[peerId]
                  .setRemoteDescription(new RTCSessionDescription(queuedOffer))
        ...
        pc.setRemoteDescription(new RTCSessionDescription(message.offer))
          ...
          .then(() => {
            ...
            if (pendingCandidates.current[peerId]) {
              console.log(
                `Applying ${pendingCandidates.current[peerId].length} pending ICE candidates`
              );
          ...
          .catch(error => {
            ...
            if (error instanceof Error && error.message.includes('m-lines in subsequent offer')) {
              ...
              setError(
                'Failed to handle connection offer: ' +
                  (error instanceof Error ? error.message : String(error))
              );
            }
          });
        ...
        pc.setRemoteDescription(new RTCSessionDescription(message.answer))
          ...
          .catch(error => {
            ...
            if (
              ...
              setError(
                'Failed to handle connection answer: ' +
                  (error instanceof Error ? error.message : String(error))
              );
            }
          });
    ...
    (message: SignalingMessage) => {
      ...
      if (peerConnections.current[peerId] && message.ice) {
        ...
        if (pc.remoteDescription && pc.remoteDescription.type) {
          ...
          pc.addIceCandidate(new RTCIceCandidate(message.ice)).catch(error => {
      ...
      const pingInterval = setInterval(() => {
        peersWithNullStreams.forEach(peerId => {
          ...
          if (
            ...
            const shouldPing =
              pingAttempt === 0 || pingAttempt === 1 || pingAttempt === 2 || pingAttempt % 5 === 0;
            ...
            if (shouldPing) {
              ...
              if (peerConnections.current[peerId] && pingAttempt % 2 === 0) {
                ...
                if (pc.signalingState === 'stable') {
                  ...
                  console.log(
                    `Connection with ${peerId} not in stable state, skipping renegotiation`
                  );
                }
              }
            }
